...
def generate_urls():
    ...
    if request.status_code==requests.codes.ok:
        for row in request.text.split('\n'):
            if 'href' in row and 'http://www.imdb.com/interfaces/' not in row:
                urls.append(row[row.index('href')+5:row.index('>', row.index('href')+5)])
    else:
...
def main():
    ...
    # Unzip files
    for f in files:
        unzipped_files.append(unzip(dir, f))
